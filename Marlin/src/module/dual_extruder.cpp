...
uint8_t DualExtruder::AutoCalibrationRightExtruder(float z_max_move_position, float speed) {
  ...
  if (z_max_move_position < Z_DEFAULT_CAIL_POSITION) {
    z_max_move_position = Z_DEFAULT_CAIL_POSITION;
...
EXIT:
  ...
  if (result == MOVE_SUCCESS) {
    // float cal_compensation_distance = z_max_move_position - tmp_position - 1.7 - 0.02; // 1.7 = (5 - 3.3)
    float cal_compensation_distance = z_max_move_position - tmp_position - 1 - 0.02; // 1.7 = (5 - 3.3)
...
uint8_t DualExtruder::MoveSensorCalibrationPosition(uint8_t mode, bool is_home) {
  ...
  switch (mode) {
    ...
    case 2:
    result = AutoCalibrationRightExtruder(Z_DEFAULT_MAX_MOVE_POSITION);
...
void DualExtruder::MoveToDestination(uint8_t *data, uint8_t data_len) {
  ...
  switch (move_type) {
    ...
    case MOVE_ASYNC:
      break;
    case MOVE_SENSOR_CALI_DEFAULT_POSITION:
    case MOVE_SENSOR_EXTRUDER_COMPRESSION:
    case MOVE_SENSOR_AUTO_CALI_COMPRESSION:
      is_cali_mode = true;
      if (data_len >= 2) {
        move_state = MoveSensorCalibrationPosition(move_type-MOVE_SENSOR_CALI_DEFAULT_POSITION);
      }
      else {
...
void DualExtruder::ReportProbeSensorCompensation() {
  ...
  uint8_t u8DataBuf[8], i, j, u8Index = 0, send_cnt = 2;
  ...
  if (RightExtruderCompensationCal(registryInstance.cfg_.probe_sensor_1_compression) == registryInstance.cfg_.probe_sensor_1_check_mark) {
    compensation[2] = registryInstance.cfg_.probe_sensor_1_compression;
    send_cnt = 3;
  }
